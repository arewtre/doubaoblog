...
class BlogCategoryController extends Controller{
    ...
    public function index(){
        $Category = NewsCategory::where("type",2)->where('pid', '0')->orderBy('descid', 'asc')->get();
        $CategoryList = [];
        if(count($Category) > 0){
            foreach($Category as $key => $value){
                $NewsCategory = NewsCategory::where("type",2)->where('pid', $value['id'])->orderBy('descid', 'asc')->get();
        ...
        //pre($CategoryList);
        return view('admin.blog.cateList', compact('CategoryList'));
    }
    ...
    public function create(Request $request){
        ...
        return view('admin.blog.cateAdd', compact('CategoryList','level'));
    }
    ...
    public function edit($id, Request $request){
        switch($request->level){
            case "0":
                ...
                return view('admin.blogcategory.editFirstClass', compact('firstClass'));
                break;
            case "1"://二级分类
                //查询父类 类别
                ...
                return view('admin.blog.cateEdit', compact('bigCategory', 'secondClass'));
                break;
        }
    }
    ...
    public function update(Request $request){
        ...
        $CategoryList = NewsCategory::where('pid', '0')->orderBy('descid', 'asc')->get();
        return view('admin.blog.cateEdit', compact('data','CategoryList'));
    }
