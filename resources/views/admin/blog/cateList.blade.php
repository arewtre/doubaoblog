...
<!--                     <button class="layui-btn layui-btn-normal " href="{{ url('admin/blogcategory/create?type=secondclass') }}"><i class="fa fa-plus"></i>添加二级类</button> -->
            ...
            @if(count($CategoryList) > 0)
                @foreach($CategoryList as $k => $v)
                    <li class="saith_bg">
                        ...
                        <span class="category-third">{{ $v->defectsname  }}</span>
                    ...
                    <button class="layui-btn layui-btn-danger layui-btn-xs"  onclick="delClass('{{$v->id}}','{{$v->level}}')">删除分类</button>
                    @if($v->level==0)
                    ...
                    @if(count($v['twoCategory']) > 0)
                        @foreach($v['twoCategory'] as $kk => $vv)
                            <li class="saith_bg double_b">
                                <span class="first">{{ $vv->descid }}</span>
                                <span class="category-third">{{ $v->html  }}{{ $vv->defectsname }}</span>
                                <span class="third">{{ $vv->new_sign }}</span>
                                <span class="four">子级</span>
                                <span class="category-five">
                                <button class="layui-btn layui-btn-xs editCate" data-id="{{$vv->id}}">修改分类</button>
                                <button class="layui-btn layui-btn-danger layui-btn-xs" onclick="delClass('{{$vv->id}}','{{$vv->level}}')">删除分类</button>
                                </span>
                            </li>
                        @endforeach
                    @endif
...
layui.use(['table','form','laydate'], function () {
	...
	$(".editCate").click(function(){
        ...
        layer.open({
            ...
            area: ['80%', '60%'],
            maxmin: true,
            content: "/admin/blog_cate_edit?id="+ id,
        });
	});
	...
	$(".addCate").click(function(){
        ...
        layer.open({
            ...
            area: ['80%', '60%'],
            maxmin: true,
            content: "/admin/blog_cate_add?pid="+ pid,
        });
	});
})
    ...
    function delClass(id, level){
        layer.confirm('您确定要删除这个分类吗？', {
            ...
            $.post("{{url('admin/blog_cate_del/')}}/" + id, {
                '_method': 'delete',
                'level': level,
                '_token': "{{csrf_token()}}"
            }, function(data){
