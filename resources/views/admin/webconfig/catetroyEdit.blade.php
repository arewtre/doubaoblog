...
<div class="result_wrap">
    <form action="{{ route('webconfig.catetroyUpdate') }}" method="post"  name="configForm" id="configForm" onsubmit="return checkForm('#configForm')">
        {{csrf_field()}}
        <table class="add_tab">
            <tbody>
            <tr>
                ...
                <td>
                    <input type="hidden" name="id" value="{{ $WebConfigCatetroy->id }}">
                    <input type="text" class="lg" name="describe" placeholder="网站配置类别中文名称" value="{{ $WebConfigCatetroy->describe }}">{{ $errors->first('describe') }}
                </td>
            </tr>
            <tr>
                <th></th>
