...
.gutter{
   border-right-color: #B9BDB6 !important; 
}
</style>
<header class="header">
	<a class="goBack fl" href="{{url("wap/news")}}">返回</a>
...
<div class="container">
	<div id="pt">
		...
		<!--{loop $cat[ups] $value}-->
			<a href="{{url('wap/news')}}">资讯/博客</a><em>&rsaquo;</em>
		<!--{/loop}-->
        ...
        <div id="related_article" class="relateItem">
            ...
            <ul class="bp" id="raid_div">
               ...
               <input type="hidden" value="{{$k+1}}">  
                 <li><a href="{{url('wap/news_'.$v->art_id.'.html')}}">{{$v->art_title}}</a></li>
              @endforeach
            </ul>
        </div>
<!-- 相关阅读结束 -->
	...
	<div class="comment_tit">
		<span class="y">已有 <strong id="_commentnum">{{$repcount}}</strong> 人参与评论</span>
		<h3>最新评论</h3>
	</div>
	<div id="comment_ul" class="pb">
		@if(count($rep)>0)
			...
			<div class="postListItem" href="#replybtn_11509" id="pid11509">
                            ...
                            <div class="postListCon">{{$v->content}}</div>
		...
		<form id="cform" name="cform" action="" method="post" autocomplete="off" class="layui-form">
                    {{csrf_field()}} 
			<textarea lay-verify="required" name="content" rows="3" class="textarea" id="message" placeholder="发表内容，参与互动"></textarea>
			<span lay-submit lay-filter="admin-form" class="btn1 mt20">评论</span>
		</form>
		<!--{/if}-->
	</div>
</div> 
...
<script type="text/javascript">
    ...
    layui.use(['layer','form','code'], function(){
      ...
      form.on('submit(admin-form)', function(data){
            ...
            $.ajax({
                type: "POST",
                url: '{{url("wap/reply")}}/'+"{{$detail->art_id}}",
                data: data.field,
