...
.postalbum_i { margin-bottom:-3px; max-width:100%; vertical-align:middle; visibility:hidden; }
	...
	<div class="postalbum">
		<div class="postalbum_h">
			<a href="{{url('wap/forum_list_detail/'.$forum->forum_id)}}" class="postalbum_h_back">返回</a>
			<span class="postalbum_h_picnum">第<span id="curpic"></span> 张 / 共 {{count($imageUrl)}} 张</span>
		</div>
		<ul class="postalbum_c">
                        @foreach($imageUrl as  $k=>$v)
			<li class="postalbum_u" id="u_{{$k}}">
				<!--{eval $imgurl = getforumimg($imglist[aid][$key], 0, 2000, 550, 'fixnone');}-->
				<img class="postalbum_i" load="0" id="img_{{$k}}" src="{{asset($v['src'])}}"zsrc="{{asset($v['src'])}}" orig="{{asset($v['src'])}}"/>
			</li>
                        @endforeach
		</ul>
	</div>
