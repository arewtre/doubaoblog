    ...
    <div class="post_from">
        <ul class="cl">
            <li>
                RE: {{$xc->xc_name}}
                @if(count($imageRep)>0)
                <div class="grey quote"><blockquote>引用: <font color="#999999">{{$imageRep->nickname}} 发表于 {{word_time($imageRep->created_at,1)}}</font><br>
                        <br>
                        <font color="#999999">{!! $imageRep->content !!}</font></blockquote>
                </div>
                @endif
            </li>
            <li class="cfix">
...
$("#postsubmit").click(function(){
    ...
    $.ajax({  
          type : "post",  
          url : "/wap/image_reply",  
          data : {content:_content,rep_id:"{{$rep_id}}",xc_id:"{{$xc_id}}",'_token':"{{csrf_token()}}"},   
          async : false,  
          success : function(res){
            if(res.code==1){
                window.location.href="{{url('wap/imageList')}}/"+res.fid;
            }
            $(this).attr("disabled",true);
          }
	});
