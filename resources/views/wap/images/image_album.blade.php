...
.postalbum_i { margin-bottom:-3px; max-width:100%; vertical-align:middle; visibility:hidden; }
	...
	<div class="postalbum">
		<div class="postalbum_h">
			<a href="{{url('wap/imageList/'.$id)}}" class="postalbum_h_back">返回</a>
			<span class="postalbum_h_picnum">第<span id="curpic">1</span> 张 / 共 {{count($imageUrl)}} 张</span>
		</div>
		<ul class="postalbum_c">
			...
			<li class="postalbum_u" id="u_{{$k}}">
				...
				<img class="postalbum_i" load="0" id="img_{{$k}}" src="{{$v->image_url}}"zsrc="{{$v->image_url}}" orig="{{$v->image_url}}"/>
			</li>
                        @endforeach
		</ul>
	</div>
