...
<div class="replyCom cfix">
	<form method="post" autocomplete="off" id="fastpostform" action="{{url('/wap/image_reply')}}">
	{{csrf_field()}}
	<ul class="fastpost">
...
<!--		<li class="cfix">
		...
		<!--{/if}-->
                <input type="hidden" value="{{$xc->xc_id}}" name="xc_id">
		<li class="cfix">
                    <textarea class="textarea input" tabindex="3" id="fastpostmessage" name="content" autocomplete="off" cols="80" rows="2" placeholder="我也说一句" ></textarea></li>
		...
		<li class="cfix">
			<div class="rePostIcon z wp50">
				<a class="faceBtn" href="javascript:void(0);"></a>
                                <a class="itBtn" href="{{url('wap/image_reply?xc_id='.$xc->xc_id)}}"></a>
			</div>
...
<!--            <td>
                <a href="{{url('wap/favorite?id='.$xc->xc_id)}}" class="viewA vFav">
...
<script type="text/javascript">
	(function() {
		...
		@if( empty(session('userinfo')) || (!empty(session('userinfo')) && $xc->allowpostreply==0) )
