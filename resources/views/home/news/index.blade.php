...
<!--   <div class="y"> 
    <a href="{{ url('feed.xml') }}" class="rss" rel="feed" target="_blank" type="application/rss+xml" title="RSS">订阅</a>
   </div> -->
  </div>
  <div class="wp"> 
    ...
    <div class="xrArticleList bp"> 
     ...
     <div class="aList newMod"> 
      ...
      @foreach($data as $v)
        ...
        <h3>
            <a class="forumName" href="{{url('news?cid='.$v->id)}}">[{{$v->defectsname}}]</a>
            <a href="{{url('news_'.$v->art_id.'.html')}}" target="_blank" style="">{{$v->art_title}}</a> 
        </h3> 
        <div class="info" style="overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:4;">
        {{$v->art_description}}
        </div> 
        <div class="attr cl"> 
    ...
    <div class="bp colu"> 
     ...
     <div class="dxb_bc" style="padding-bottom:20px;"> 
      <ul class="module cl xl xl2 cl">
       @foreach($ncate as $v)
          <li @if($cid==$v->id) class="cur" @endif> <a href="{{url('news?cid='.$v->id)}}">{{$v->defectsname}}</a></li> 
       @endforeach
      </ul> 
     </div> 
    </div> 
...
//完整功能
  laypage.render({
    ...
    ,jump: function(obj,first){
      if(!first){   
            window.location.href="/news?page="+obj.curr+"&cid="+"{{$cid}}";
        }  
    }
  });
