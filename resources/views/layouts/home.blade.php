...
<head>
    ...
    <meta name="keywords" content="豆宝,豆豆宝,豆宝网" /> 
    <meta name="description" content="豆宝,豆豆宝,豆宝网" /> 
...
jQuery(document).ready(function(){
    ...
    console.log(mTop);
    if(jQuery(window).scrollTop()>=mTop+300){
        jQuery("#hd").addClass("fixed");
    }else{
        jQuery("#hd").removeClass("fixed");
    }
    });
//    jQuery("body,html").scrollTop(20);
...
.xr_tl #fd_page_bottom .pg strong, .xr_tl #fd_page_bottom .pg a:hover, .pgt .pg strong, .pgt .pg a:hover, .pgs .pg strong, .pgs .pg a:hover {
    background: #FF7F84;
    border-color: #FF7F84;
    color: #fff;
}
...
<!--     <div id="scbar" class="cl y"> 
      <form id="scbar_form" method="post" autocomplete="off" action="{{url('search')}}" target="_blank"> 
         {{csrf_field()}} 
         <input type="hidden" name="mod" id="scbar_mod" value="" /> 
       <ul> 
        <li class="scbar_type"><a href="javascript:;" id="scbar_type" onclick="showMenu(this.id)" hidefocus="true">资讯</a></li> 
        <li class="scbar_txt">
            <input type="text" name="keywords" id="scbar_txt" value="" autocomplete="off" />
        </li> 
        <li class="scbar_btn">
            <button type="submit" name="searchsubmit" id="scbar_btn" sc="1" value="true">
